...
let WigaClass = Wiga.class({
        ...
        WigaTable.init({
            ...
            columns: [
                ...
                { data: function(data){ 
                    ...
                    let alertCert = data.certificates.length == 0? '<i class="fas fa-exclamation-triangle text-warning" data-bs-toggle="tooltip" title="Belum ada sertifikat"></i>' : '';
                    let type = `<span class="badge badge-light-${data.type == 'offline' ? 'danger' : 'success'}">${data.type.toUpperCase()}</span>`
                    return `${alertCert} <a href="${WigaRoute.url('event/'+data.slug)}" class="fw-bold mb-2">${data.title}</a><br>${type}`
                }},
               { 
                    data: function(data) {
                        if (data.categories && data.categories.length > 0) {
                            ...
                            // Ambil hanya 3 kategori pertama
                            let badges = data.categories.slice(0, maxVisible).map(cat => {
                                return `<span class="badge badge-light-primary mb-1 me-1">${cat.name}</span>`;
                            }).join('');

                            // Jika ada lebih dari 3, tambahkan indikator "+ n lainnya"
                            if (totalCategories > maxVisible) {
                                let remaining = totalCategories - maxVisible;
                                badges += `<span class="badge badge-light-secondary mb-1" title="Sisa kategori tidak ditampilkan">+${remaining} lainnya</span>`;
                            }
                ...
                { data: function(data){
                    ...
                    // if(data.status_id == 0){
                        ...
                        const userActions = [
                            ...
                            { 
                                text: '<span class="badge badge-light-primary fw-bold border-start border-end border-primary">Template Sertifikat</span>',
                                className: 'fw-bold',
                                href: WigaRoute.url('/portal/events/'+data.id+'/certificates')
                                // events: {
                                //     click: function(e) {
                                //         e.preventDefault();
                                //         WigaRoute.redirect('/portal/events/'+data.id+'/participants');
    ...
    async generateAttendance(id)
    {
        ...
        $wiga('#ModalGenerateAttendance #attendance-loading').removeClass('d-none');
        $wiga('#ModalGenerateAttendance #attendance-content').addClass('d-none');
        ...
        WigaHttp.handle(response,null,function(res) {
            ...
            $wiga('#ModalGenerateAttendance #attendance-loading').addClass('d-none');
            $wiga('#ModalGenerateAttendance #attendance-content').removeClass('d-none');
