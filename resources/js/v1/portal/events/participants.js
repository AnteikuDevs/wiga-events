...
let WigaClass = Wiga.class({
    render()
    {
        ...
        WigaTable.init({
            ...
            columns: [
                ...
                { data: function(data){
                    return `<span class="badge badge-light-info">${data.certificate_as}</span>` + (data.certificate_template.name ? `<br><small class="text-muted">${data.certificate_template.name} - Model ${data.certificate_template.model_id}</small>` : '')
                } },
                { data: function(data){
                    ...
                    userActions.push({
                        text: '<span class="badge badge-light-info">Model Sertifikat</span>', 
                        events: {
                            click: function(e) {
                                
                                $wiga('#ModalCert').attr('data-id',data.id);
                                $wiga('#ModalCert [name=certificate_template]').val(data.certificate_template_id).trigger('change');
                                $wiga('#ModalCert [name=certificate_as]').val(data.certificate_as);
                                $wiga('#ModalCert').modal('show');
    ...
    async certTemplate()
    {
        let response = await WigaHttp.get('/portal/events/certificates/list',{event: WigaRoute.segment(3)});
        if(response.status)
        {
            let data = response.data.map(function(val){
                return {
                    id: val.id,
                    text: val.name,
                    description: "Model " +val.model_id
                }
            })

            $('#ModalCert [name=certificate_template]').select2({
                placeholder: 'Pilih Model Sertifikat',
                data: data,
                dropdownParent: $('#ModalCert'),
    ...
    async changeModel()
    {
        let data = WigaForm.data('#ModalCert')
        let response = await WigaHttp.post('/portal/events/'+eventID+'/participants'+'/'+$wiga('#ModalCert').attr('data-id')+'/model',data);
        WigaHttp.handle(response,'#ModalCert form',function(res) {
            $wiga('#ModalCert').modal('hide');
            WigaClass.render();
            WigaNotify.showInline('#wiga-alert',{
                type: 'success',
                content: res.message,
            ...
            WigaNotify.showInline('#ModalCertAlert',{
                type: 'danger',
                content: res.message,
            })
        });
...
$wiga('#ModalCert [type="submit"]').on('click',function(e){
    e.preventDefault();
    $wiga('#ModalCert [type="submit"]').indicator(WigaClass.changeModel());    
})